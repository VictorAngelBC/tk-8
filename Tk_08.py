...
EQUIPO     = [
    'Integrante 1  —  Avila Enriquez Ignacio',
    'Integrante 2  —  Barrueta Carbajal Victor Angel',
    'Integrante 3  —  Mayorga Ruiz Emmanuel Eusebio',
    'Integrante 4  —  Padron Mendoza Jesus Ismael',
    'Integrante 5  —  Guerrero Mendoza Emmanuel',
    'Integrante 6  —  Sierra Ramirez Nubia Janeth',
]
INSTITUCION = 'UAT — Unidad Académica Multidisciplinaria Mante  |  ISC 2026-1'
...
class Dashboard:
    ...
    def _construir(self):
        ...
        tk.Label(top, text='  Dashboard HMI — Monitoreo de Proceso',
        ...
        info_bar = tk.Frame(self.root, bg='#0d1520', height=36)
        info_bar.pack(fill=tk.X)
        info_bar.pack_propagate(False)

        tk.Label(info_bar, text=MATERIA,
            bg='#0d1520', fg=ACE, font=('Arial', 8, 'bold')
        ).pack(side=tk.LEFT, padx=12, pady=9)

        tk.Label(info_bar, text='|', bg='#0d1520', fg=T3,
            font=('Arial', 8)).pack(side=tk.LEFT, padx=4)

        # Filtrar integrantes con nombre completo
        miembros = [e.split('—')[-1].strip() for e in EQUIPO if e.split('—')[-1].strip()]
        miembros_str = '   ·   '.join(miembros)
        tk.Label(info_bar, text=miembros_str,
            bg='#0d1520', fg=T2, font=('Arial', 7)
        ).pack(side=tk.LEFT, padx=4)

        tk.Label(info_bar, text=INSTITUCION,
            bg='#0d1520', fg=T3, font=('Arial', 7)
        ...
        for key, nombre, uni, color in [
            ('temp','TEMPERATURA','C',RJO),('pres','PRESION','hPa',ACE),
        ...
        self.lbl_tmax = tk.Label(izq,text='Max: --',bg=BG2,fg=AMR,font=('Arial',9))
        self.lbl_tmax.pack(anchor='w',padx=10)
        self.lbl_tmin = tk.Label(izq,text='Min: --',bg=BG2,fg=ACE,font=('Arial',9))
        ...
        self.txt_alm  = cv_alm.create_text(50,25,text='Sin alarma',
            fill=T3, font=('Arial',10,'bold'), anchor='w')
        self.cv_alm = cv_alm
    ...
    def _poll(self):
        ...
        if not m.q_datos.empty():
            ...
            self.lbl_tmax.config(text=f'Max: {m.t_max:.1f} C')
            self.lbl_tmin.config(text=f'Min: {m.t_min:.1f} C')
            # Alarma
            if m.alarma:
